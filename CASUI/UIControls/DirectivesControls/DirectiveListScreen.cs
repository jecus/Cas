...
namespace CAS.UI.UIControls.DirectivesControls
{
    ...
    ///</summary>
    [ToolboxItem(false)]
    public class DirectiveListScreen : UserControl, IReference
    {
        #region Fields
        ...
        #region public DirectiveListScreen(BaseDetail currentBaseDetail, DirectiveCollectionFilter viewFilter, DirectiveType directiveType, string reportText, SDList<BaseDetailDirective> directiveList)
        ...
        public DirectiveListScreen(BaseDetail currentBaseDetail, DirectiveCollectionFilter viewFilter, 
        ...
        #region public DirectiveListScreen(Aircraft currentAircraft, DirectiveCollectionFilter viewFilter, DirectiveType directiveType, string reportText, SDList<BaseDetailDirective> directiveList)
        ...
        ///<param name="reportText">Текст отчета</param>
        ///<param name="directiveList"></param>
        public DirectiveListScreen(Aircraft currentAircraft, DirectiveCollectionFilter viewFilter, DirectiveCollectionFilter additionalFilter,
                                   DirectiveType directiveType, string reportText,
                                   SDList<BaseDetailDirective> directiveList)
        {
            ...
            InitListView(directiveList);
            filterSelection = new DirectiveFilterSelectionForm(directiveType, additionalFilter);
...
/*            // 
            ...
            // 
            toolStripMenuItemView.Text = "Open";
            ...
            headerControl.ResumeLayout(false);
            headerControl.PerformLayout();
            ResumeLayout(false);
            PerformLayout();
        }
        ...
        private void CopyDirectives()
        {
            try
            {
                if (CASClipboard.Instance.Contents.Count > 0 && CASClipboard.Instance.Contains(typeof(BaseDetailDirective)))
                {
                    for (int i = 0; i < CASClipboard.Instance.Contents.Count; i++)
                    {
                        if (CASClipboard.Instance.Contents[i] is BaseDetailDirective)
                        {
                            BaseDetailDirective directiveLink = (BaseDetailDirective)CASClipboard.Instance.Contents[i];
                            BaseDetailDirective directive = directiveLink.DeepCopy();
                            if (currentAircraft != null)
                                currentAircraft.Add(directive);
                            else
                                currentBaseDetail.Add(directive);
                        }
                    }
        ...
        private void InitListView(SDList<BaseDetailDirective> directiveList)
        {
            ...
            directivesViewer.ItemsPasted += directivesViewer_ItemsPasted;
            directivesViewer.ItemsDeleted += ButtonDelete_Click;
        ...
        private void UpdateDirectives()
        {
            ...
            //currentAircraft.Limitation.ResourceSinceNew.ToComplianceItemString();todo
            else if (currentBaseDetail != null)
                labelTSNCSN.Text = "     " + currentBaseDetail + " TSN/CSN: ";
                                   //currentBaseDetail.Limitation.ResourceSinceNew.ToComplianceItemString(); todo
            ...
            try
            {
                ...
                if (directiveAdded <= 0)
                {
                    if (directivesViewer.SelectedItems.Count == 1)
                        message = "Directive is already added to the work package or closed";
                    else
                        message = "Directives are already added to the work package or closed";
                }
                else if (directiveAdded == 1)
                    message = "Directive added successfully";
                else
                    message = "Directives added successfully";
                if (
...
*/
        ...
        private void HighlightItem_Click(object sender, EventArgs e)
        {
            for (int i = 0; i < directivesViewer.SelectedItems.Count; i++)
            {
                directivesViewer.SelectedItems[i].Highlight = (Highlight)((ToolStripMenuItem)sender).Tag;

                try
                {

                    directivesViewer.SelectedItems[i].Save(true);
                }
                catch (Exception ex)
                {
                    Program.Provider.Logger.Log("Error while saving data", ex);
        ...
        private void toolStripMenuItemCopy_Click(object sender, EventArgs e)
        {
            ...
            if (count > 0)
            {
                ...
                for (int i = 0; i < count; i++)
                    contents.Add(directivesViewer.SelectedItems[i]);
        ...
        #region private void directivesViewer_ItemsPasted(List<BaseDetailDirective> pastedItems)

        private void directivesViewer_ItemsPasted(List<BaseDetailDirective> pastedItems)
        {
            CopyDirectives();
        }
        ...
        private void CheckContextMenu(int count)
        {
            ...
            toolStripMenuItemPaste.Enabled = CASClipboard.Instance.Contains(typeof (BaseDetailDirective));
        ...
        private void HookDirective(BaseDetailDirective directive)
        {
            ...
            directive.Changed += DirectiveListControl_Changed;
            directive.RecordsChanged += DirectiveListControl_Changed;
        ...
        #region private void DirectiveListControl_Changed(object sender, EventArgs e)

        private void DirectiveListControl_Changed(object sender, EventArgs e)
        {
            directivesViewer.EditItem(null, (BaseDetailDirective) sender);
            GatherDirectives();
        }
