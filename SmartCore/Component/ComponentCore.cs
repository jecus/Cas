...
namespace SmartCore.Component
{
    public class ComponentCore : IComponentCore
    {
        ...
        {
            return _newLoader.GetObjectListAll<ComponentDTO, Entities.General.Accessory.Component>(
        ...
        public ComponentCollection GetComponents(params object[] parametres)
        {
            ...
            throw new ArgumentException("must be Aircraft or BaseComponent or Store or List<BaseComponent>",
                "parametres");
        }
        ...
        {
            var directives = _newLoader
                .GetObjectListAll<ComponentDirectiveDTO, ComponentDirective>(
                    new Filter("ComponentId", baseComponent.ItemId), loadChild).ToList();
            ...
            var itemsRelations =
                _itemsRelationsDataAccess.GetRelations(directivesIds, SmartCoreType.ComponentDirective.ItemId);

            foreach (var componentDirective in directives)
            {
                componentDirective.ParentComponent = baseComponent;
                componentDirective.ItemRelations.AddRange(itemsRelations.Where(i =>
                    i.FirstItemId == componentDirective.ItemId ||
                    i.SecondItemId == componentDirective.ItemId)); //TODO:(Evgenii Babak)не использовать Where 
            }
        ...
        public List<ComponentDirective> GetComponentDirectives(IList<int> componentDirectiveIds,
            bool getDeleted = false)
        {
            ...
            componentDirectives.AddRange(
                _newLoader.GetObjectListAll<ComponentDirectiveDTO, ComponentDirective>(
                    new Filter("ItemId", componentDirectiveIds), true, getDeleted));
            ...
            var components = _newLoader.GetObjectListAll<ComponentDTO, Entities.General.Accessory.Component>(
                new List<Filter>()
                {
                    new Filter("IsBaseComponent", false),
        ...
        public List<ComponentDirective> GetAircraftComponentDirectives(int aircraftId, IList<int> componentDirectiveIds,
            bool getDeleted = false)
        {
            var componentDirectives = new List<ComponentDirective>();
            ...
            if (itemsRelations.Count > 0)
            {
                foreach (var componentDirective in componentDirectives)
                    componentDirective.ItemRelations.AddRange(
                        itemsRelations.Where(i =>
                            i.FirstItemId == componentDirective.ItemId || i.SecondItemId == componentDirective.ItemId));
        ...
        private ComponentCollection GetComponentsByQuery(string query, bool loadChild = true)
        {
            var components =
                new ComponentCollection(
                    _loader.GetObjectListAll<Entities.General.Accessory.Component>(query, loadChild));
        ...
        {
            var filter = new CommonFilter<string>(
                $@"({SmartCoreType.Supplier.ItemId} in (select top 1 [destinationobjecttype] 
            ...
            var qrs = BaseQueries.GetSelectQueryWithWhere<Entities.General.Accessory.Component>(allFilters.ToArray(),
                loadChild);
        ...
        {
            var filter = new CommonFilter<string>(
                $@"({SmartCoreType.Employee.ItemId} in (select top 1 [destinationobjecttype] 
                                        from dbo.TransferRecords 
        ...
        public void AddComponentDirective(Entities.General.Accessory.Component component,
            ComponentDirective componentDirective)
        {
            ...
            if (component.ItemId <= 0)
                throw new Exception("1026: Can not add component directive to not existing component");
        ...
        public void AddComponent(Entities.General.Accessory.Component component, IComponentContainer destination,
            DateTime installationDate, string position, ComponentStorePosition state,
            Lifelength installationLifelength = null, Lifelength lastKnownLifelength = null,
            DateTime? lastKnownLifelenghtDate = null,
            bool destinationResponsible = false)
        {
            // Небольшая проверка
            if (destination.ItemId <= 0)
                throw new Exception("1537: Can not add component to not existing base component");
        ...
        public void UpdateComponent(Entities.General.Accessory.Component component, DateTime installationDate,
            string position, ComponentStorePosition state, Lifelength installationLifelength)
        {
            if (component == null)
            ...
            bool destinationResponsible = false, bool reload = true)
        {
            if (baseComponent == null)
                throw new Exception("0946: Can not save base component to not existing aircraft");
        ...
        public void AddBaseComponent(BaseComponent baseComponent, Store store, DateTime date, String position,
            Lifelength installationData)
        {
        ...
        public void DeleteLLPCategoryChangeRecord(ComponentLLPCategoryChangeRecord componentLLPCategoryChangeRecord)
        {
            ...
            {
                componentLLPCategoryChangeRecord.ParentComponent.ChangeLLPCategoryRecords.Remove(
                    componentLLPCategoryChangeRecord);
                ...
                foreach (var component in components)
                {
                    var llp = component.ChangeLLPCategoryRecords.FirstOrDefault(i =>
                        i.ToCategory.ItemId == componentLLPCategoryChangeRecord.ToCategory.ItemId &&
                        i.RecordDate.Equals(componentLLPCategoryChangeRecord.RecordDate));
                    if (llp != null)
        ...
        public void DeleteBaseComponent(BaseComponent baseComponent)
        {
            ...
            if (baseComponent.BaseComponentType == BaseComponentType.Frame)
                throw new Exception("1129: Can not delete frame base component");
        ...
        public void RegisterActualState(BaseEntityObject entityObject, DateTime actualDate, Lifelength actualState)
        {
            if (entityObject == null)
                throw new Exception("1539: Can not register actual state record for not existing object");
            if (entityObject.ItemId <= 0)
                throw new Exception("1540: Can not register actual state record for not existing object");
            if (actualDate < DateTimeExtend.GetCASMinDateTime())
                throw new Exception("1540: Can not register actual state record for date less 1950,1,1");
            if (actualState == null ||
                actualState.IsNullOrZero())
                throw new Exception("1540: Can not register actual state record for empty lifelength");
            if (entityObject is Aircraft)
            {
                var aircraftFrame = getBaseComponentById(((Aircraft)entityObject).AircraftFrameId);
                registerActualState(aircraftFrame,
                    new ActualStateRecord { RecordDate = actualDate, OnLifelength = actualState });
            }
            else if (entityObject is BaseComponent)
                registerActualState((BaseComponent)entityObject,
                    new ActualStateRecord { RecordDate = actualDate, OnLifelength = actualState });
            else if (entityObject is Entities.General.Accessory.Component)
                registerActualState((Entities.General.Accessory.Component)entityObject,
                    new ActualStateRecord { RecordDate = actualDate, OnLifelength = actualState });
        }
        ...
        private void registerActualState(Entities.General.Accessory.Component component, ActualStateRecord actualState)
        {
            // Не можем добавить актуальное состояние не существующему агрегату
            if (component.ItemId <= 0)
                throw new Exception("1009: Can not register actual state for not existing component");
            if (component == null)
                throw new Exception("1610: Can not register actual state for not existing component");
            ...
            var existing = component.ActualStateRecords[actualState.RecordDate.Date,
                actualState.WorkRegime ?? FlightRegime.UNK];
            if (existing != null)
            {
        ...
        public void RegisterChangeLLPCategory(Entities.General.Accessory.Component component,
            ComponentLLPCategoryChangeRecord performance)
        {

            // Не можем добавить выполнение для не существующей директивы
            if (component.ItemId <= 0)
                throw new Exception("1033: Can not register performance for not existing component directive");
        ...
        public void MoveToStore(Entities.General.Accessory.Component component, Store store, DateTime transferDate,
            double beforeReplace, double replaced, Lifelength baseComponentLlOnTransferDate, string description,
            string remarks, InitialReason reason, Specialist released, Specialist received, WorkPackage wp = null,
            AttachedFile transferRecordAttachedFile = null)
        {
            ...
            if (beforeReplace < replaced)
                throw new ArgumentNullException("beforeReplace",
                    "The number of parts to move was less than the number of parts transported");
            ...
            if (beforeReplace != replaced)
            {
                ...
                var newComponent = new Entities.General.Accessory.Component(component)
                    { Quantity = replaced, QuantityIn = replaced };
                ...
                if (newComponent.TransferRecords.Any(transferRecord =>
                        transferRecord.TransferDate == newTransferRecord.TransferDate))
        ...
        public void MoveToProcessing(Entities.General.Accessory.Component component, int destinationSupplierId,
            int destinationSpecialistId, DateTime transferDate, double beforeReplace,
            double replaced, Lifelength baseComponentLlOnTransferDate, string description, string remarks,
            InitialReason reason,
            Specialist released, Specialist received, WorkPackage currentWorkPackage,
            AttachedFile transferRecordAttachedFile, DateTime receipDate, Lifelength notify)
        {
            if (destinationSupplierId <= 0 && destinationSpecialistId <= 0)
                throw new Exception("1527: Can not move component to not existing specialist or supplier");
            ...
            var newTransferRecord = new TransferRecord
            {
                ...
                AttachedFile =
                    currentWorkPackage != null ? currentWorkPackage.AttachedFile : transferRecordAttachedFile,
                Description = description,
                Reason = reason,
                ReceivedSpecialist = received,
                ReleasedSpecialist = released,
        ...
        /// <param name="wp"></param>
        /// <param name="transferRecordAttachedFile"></param>
        public void MoveToAircraft(Entities.General.Accessory.Component component, BaseComponent baseComponent,
            Aircraft aircraft, DateTime transferDate,
            double beforeReplace, double replaced, Lifelength baseComponentLlOnTransferDate, string description,
            InitialReason reason, Specialist released, Specialist received, WorkPackage wp = null,
            AttachedFile transferRecordAttachedFile = null)
        {
            if (aircraft.ItemId <= 0) throw new Exception("1519: Can not move component to not existing aircraft");
            if (getBaseComponentById(aircraft.AircraftFrameId) == null)
                throw new Exception("1520: Can not move component to not existing aircraft");
            if (aircraft.AircraftFrameId <= 0)
                throw new Exception("1521: Can not move component to not existing aircraft");
            if (baseComponent.ItemId <= 0) throw new Exception("1518: Can not move to not existing base component");
            MoveToBaseComponent(component, baseComponent, transferDate, beforeReplace, replaced,
                baseComponentLlOnTransferDate, description, reason, released, received, wp, transferRecordAttachedFile);
        }
        ...
        /// <param name="wp"></param>
        /// <param name="transferRecordAttachedFile"></param>
        public void MoveToStore(BaseComponent baseComponent, Store store, DateTime transferDate,
            Lifelength baseComponentLlOnTransferDate, string description, InitialReason reason, Specialist released,
            Specialist received,
            WorkPackage wp = null, AttachedFile transferRecordAttachedFile = null)
        {
            ...
            if (baseComponent.BaseComponentType == BaseComponentType.Frame)
                throw new Exception("1803: Can not move frame base components");
            ...
            // проверяем есть ли пемещения на данную дату
            if (baseComponent.TransferRecords.Any(transferRecord =>
                    transferRecord.TransferDate == newTransferRecord.TransferDate))
                throw new Exception("1658: There can not be several transfer records at one day");
        ...
        /// <param name="wp"></param>
        /// <param name="transferRecordAttachedFile"></param>
        public void MoveToAircraft(BaseComponent baseComponent, Aircraft aircraft, DateTime transferDate,
            Lifelength baseComponentLlOnTransferDate, string description, InitialReason reason, Specialist released,
            Specialist received, WorkPackage wp = null, AttachedFile transferRecordAttachedFile = null)
        {
            ...
            // проверяем есть ли пемещения на данную дату
            if (baseComponent.TransferRecords.Any(
                    transferRecord => transferRecord.TransferDate == transfer.TransferDate))
                throw new Exception("1658: There can not be several transfer records at one day");
        ...
        /// <param name="wp"></param>
        /// <param name="transferRecordAttachedFile"></param>
        private void MoveToBaseComponent(Entities.General.Accessory.Component component, BaseComponent destination,
            DateTime transferDate,
            double beforeReplace, double replaced, Lifelength baseComponentLlOnTransferDate, string description,
            InitialReason reason, Specialist released, Specialist received, WorkPackage wp,
            AttachedFile transferRecordAttachedFile = null)
        {
            ...
            var transfer = new TransferRecord
            {
                ...
                FromBaseComponentId = component.ParentBaseComponent != null
                    ? component.ParentBaseComponent
                        .ItemId //TODO:(Evgenii Babak) заменить на использование ComponentCore 
                    : 0,
                FromStoreId = component.ParentStoreId,
                FromSupplierId = component.ParentSupplierId,
                FromSpecialistId = component.ParentSpecialistId,
            ...
            if (beforeReplace != replaced)
            {
                ...
                // проверка есть ли пемещения на данную дату
                if (newComponent.TransferRecords.Any(transferRecord =>
                        transferRecord.TransferDate == transfer.TransferDate))
                    throw new Exception("1658: There can not be several transfer records at one day");
                // добавляем в коллекцию
                //foreach (var tr in newComponent.TransferRecords)
                //{
        ...
        private static void Check(Entities.General.Accessory.Component component, TransferRecord transferRecord)
        {
            if (transferRecord.ItemId > 0)
                if (transferRecord.TransferDate > component.ActualStateRecords[0].RecordDate.Date)
                    throw new Exception(
                        "953: You can not change the date on TransferRecord bigger than the first ActualState");
        }
        ...
        public void LoadRelatedObjectds(Entities.General.Accessory.Component[] components, bool loadChild = true)
        {
            ...
            var transfers = _newLoader
                .GetObjectListAll<TransferRecordDTO, TransferRecord>(new Filter("ParentID", componentids), false)
                .ToList();
            ...
            var actuals =
                _newLoader.GetObjectListAll<ActualStateRecordDTO, ActualStateRecord>(new Filter("ComponentId",
                    componentids));
            ...
            if (loadChild)
            {
                ...
                var llpData =
                    _newLoader.GetObjectListAll<ComponentLLPCategoryDataDTO, ComponentLLPCategoryData>(
                        new Filter("ComponentId", componentids));
                ...
                var llpChangeRecords =
                    _newLoader.GetObjectListAll<ComponentLLPCategoryChangeRecordDTO, ComponentLLPCategoryChangeRecord>(
                        new Filter("ParentId", componentids));
                ...
                var componentIds = components.Select(d => d.ItemId).ToArray();
                var directives =
                    _newLoader.GetObjectList<ComponentDirectiveDTO, ComponentDirective>(
                        new Filter("ComponentId", componentIds), true);
                    ...
                    if (itemsRelations.Count > 0)
                    {
                        foreach (var componentDirective in directives)
                            componentDirective.ItemRelations.AddRange(itemsRelations.Where(i =>
                                i.FirstItemId == componentDirective.ItemId ||
                                i.SecondItemId ==
                                componentDirective.ItemId)); //TODO:(Evgenii Babak)не использовать Where 
                    }
                }
                ...
                foreach (var component in components)
                {
                    ...
                    {
                        component.ChangeLLPCategoryRecords.AddRange(llpChangeRecords
                            .Where(k => k.ParentId == component.ItemId).ToArray());
        ...
        {
            var component =
                _loader.GetObject<Entities.General.Accessory.Component>(componentId, true, getDeleted, true);
            ...
            {
                var crs =
                    _casEnvironment.GetDictionary<DocumentSubType>().GetByFullName("Component CRS Form") as
                        DocumentSubType;
                var shipping =
                    _casEnvironment.GetDictionary<DocumentSubType>().GetByFullName("Shipping document") as
                        DocumentSubType;
        ...
        public IList<Entities.General.Accessory.Component> GetComponentByIds(IList<int> componentIds)
        {
            ...
            components.AddRange(_newLoader.GetObjectList<ComponentDTO, Entities.General.Accessory.Component>(
                new List<Filter>()
                {
                    new Filter("IsBaseComponent", false),
                    new Filter("ItemId", componentIds)
        ...
        public BaseComponent GetFullBaseComponent(int baseComponentId)
        {
            ...
            var directives =
                _newLoader.GetObjectList<ComponentDirectiveDTO, ComponentDirective>(
                    new Filter("ComponentId", baseComponentId), true);
        ...
        {
            var baseComponents = new List<BaseComponent>(aircraft != null
                ? getAicraftBaseComponents(aircraft.ItemId)
                : _casEnvironment.BaseComponents.ToArray());
            foreach (var baseComponent in baseComponents)
                baseComponent.ComponentDirectives.Clear();
            var ids = baseComponents.Select(bd => bd.ItemId).ToArray();

            var componentDirectives =
                _newLoader.GetObjectList<ComponentDirectiveDTO, ComponentDirective>(new Filter("ComponentId", ids),
                    true);
        ...
        {
            var ids = _newLoader.GetSelectColumnOnly<BaseComponentDTO>(new[] { new Filter("IsBaseComponent", true) },
                "ItemId");
            var actuals =
                _newLoader.GetObjectListAll<ActualStateRecordDTO, ActualStateRecord>(new Filter("ComponentId", ids));
        ...
        public BaseComponent[] GetAicraftBaseComponents(int aircraftId, int? baseComponentTypeId = null)
        {
            ...
            return _casEnvironment.BaseComponents.Where(candidate => candidate.ParentAircraftId == aircraftId
                                                                     && candidate.BaseComponentType.ItemId ==
                                                                     baseComponentTypeId).ToArray();
        }
        ...
        {
            var bs = _casEnvironment.BaseComponents.Where(candidate => candidate.ParentAircraftId == aircraftId)
                .ToList();
        ...
        public BaseComponent[] GetAicraftBaseComponents(int aircraftId, IEnumerable<int> baseComponentTypeIds)
        {
            ...
            return _casEnvironment.BaseComponents.Where(candidate => candidate.ParentAircraftId == aircraftId
                                                                     && baseComponentTypeIds.Contains(
                                                                         candidate.BaseComponentTypeId)).ToArray();
        }
        ...
        public BaseComponent[] GetStoreBaseComponents(int storeId, int? baseComponentTypeId = null)
        {
            ...
            return _casEnvironment.BaseComponents.Where(candidate =>
                    candidate.ParentStoreId == storeId && candidate.BaseComponentType.ItemId == baseComponentTypeId)
                .ToArray();
        }
        ...
        public BaseComponent[] GetOperatorBaseComponents(int operatorId, int? baseComponentTypeId = null)
        {
            ...
            return _casEnvironment.BaseComponents.Where(candidate => candidate.ParentOperator.ItemId == operatorId
                                                                     && candidate.BaseComponentType !=
                                                                     BaseComponentType.Frame
                                                                     && candidate.BaseComponentType.ItemId ==
                                                                     baseComponentTypeId).ToArray();
        }
        ...
        public BaseComponent GetAircraftFrame(int aircraftId)
        {
            ...
            return _casEnvironment.BaseComponents.SingleOrDefault(bd =>
                bd.ParentAircraftId == aircraftId && !bd.IsDeleted &&
                bd.BaseComponentTypeId == BaseComponentType.Frame.ItemId);
        }
        ...
        private void ConnectComponentDirectivesWithComponents(
            IEnumerable<Entities.General.Accessory.Component> components, IEnumerable<ComponentDirective> directives)
        {
        ...
        private void SetFromAndDestination(IEnumerable<TransferRecord> transfers)
        {
            ...
            suppliersIds.AddRange(transfers.Where(t => t.DestinationObjectType == SmartCoreType.Supplier)
                .Select(t => t.DestinationObjectId));
            ...
            specialistIds.AddRange(transfers.Where(t => t.DestinationObjectType == SmartCoreType.Employee)
                .Select(t => t.DestinationObjectId));
            specialistIds.AddRange(transfers.Select(t => t.FromSpecialistId));
            var specialists =
                _newLoader.GetObjectListAll<SpecialistDTO, Specialist>(new Filter("ItemId", specialistIds));
        ...
        private void ConnectTransfersWithComponents(IEnumerable<Entities.General.Accessory.Component> components,
        ...
        private void ConnectActualsWithComponents(IEnumerable<Entities.General.Accessory.Component> components,
            IEnumerable<ActualStateRecord> actuals)
        {
        ...
        /// <param name="component"></param>
        /// <returns></returns>
        public void SetDestinations(Entities.General.Accessory.Component component)
        {
            ...
            if (lastTransfer.DestinationObjectType == SmartCoreType.Store)
            {
                ...
                if (store == null)
                {
                    // перемещение вникуда !
                    throw new Exception(
                        $"1552: Destination object ID:{lastTransfer.DestinationObjectId} was not found");
                }
            ...
            {
                var baseComponent =
                    _casEnvironment.BaseComponents.GetItemById(lastTransfer
                        .DestinationObjectId); //TODO(Evgenii Babak): использовать ComponentCore
                if (baseComponent != null)
                {
            ...
            if (component.ParentStoreId <= 0 && component.ParentAircraftId <= 0 && component.ParentSupplierId <= 0 &&
                component.ParentSpecialistId <= 0 && component.ParentOperator == null)
        ...
        public void LoadBaseComponents(Aircraft aircraft)
        {
            ...
            var bc = _newLoader.GetObjectListAll<BaseComponentDTO, BaseComponent>();
            ...
            var directives = _newLoader.GetObjectList<ComponentDirectiveDTO, ComponentDirective>(new Filter("ComponentId", baseComponentIds), true);
