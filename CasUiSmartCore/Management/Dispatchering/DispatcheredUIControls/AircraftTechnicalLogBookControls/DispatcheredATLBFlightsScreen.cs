...
namespace CAS.UI.Management.Dispatchering.DispatcheredUIControls.AircraftTechnicalLogBookControls
{
    ...
    /// </summary>
    [ToolboxItem(false)]
    public class DispatcheredATLBFlightsScreen : ATLBFlightsScreen, IDisplayingEntity
    {
        ...
        {
            if (!(obj is DispatcheredATLBFlightsScreen))
                return false;
            if (!(obj.ContainedData is ATLB))
                return false;

            return (currentATLB.ID == ((ATLB)obj.ContainedData).ID);
        }
