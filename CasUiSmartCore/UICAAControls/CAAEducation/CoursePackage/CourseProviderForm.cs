...
{
	public partial class CourseProviderForm : MetroForm
	{
		#region Fields

		private readonly List<Supplier> _suppliers = new List<Supplier>();
		private List<CourseProviderPrice> _prices = new List<CourseProviderPrice>();
		private SmartCore.CAA.CAAWP.CoursePackage _selectedItem;

		#endregion

		#region Constructor

		public CourseProviderForm()
		{
			InitializeComponent();
		}

		public CourseProviderForm(SmartCore.CAA.CAAWP.CoursePackage selectedItem) : this()
		{
			_selectedItem = selectedItem;

			foreach (var price in selectedItem.Settings.ProviderPrice)
				price.Supplier = _suppliers.FirstOrDefault(i => i.ItemId == price.SupplierId);

			_prices.AddRange(selectedItem.Settings.ProviderPrice);
			if (_prices.Count > 0)
		...
		{
			_suppliers.Clear();
			_suppliers.AddRange(GlobalObjects.CaaEnvironment.NewLoader.GetObjectListAll<CAASupplierDTO, Supplier>(loadChild: true));
		...
		private void ButtonAdd_Click(object sender, System.EventArgs e)
		{
			foreach (var supplier in supplierListView.SelectedItems.ToArray())
				_prices.Add(new CourseProviderPrice { Supplier = supplier, SupplierId = supplier.ItemId, Parent = _selectedItem });
		...
		private void SupplierListView1_SelectedItemsChanged(object sender, UIControls.Auxiliary.SelectedItemsChangeEventArgs e)
		{
		...
		private void ButtonOk_Click(object sender, System.EventArgs e)
		{
			...
			_selectedItem.Settings.ProviderPrice.Clear();
			_selectedItem.Settings.ProviderPrice.AddRange(_prices);
		}
