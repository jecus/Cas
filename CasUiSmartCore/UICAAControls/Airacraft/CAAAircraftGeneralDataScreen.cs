...
namespace CAS.UI.UICAAControls.Airacraft
{
    ///<summary>
    ///</summary>
    public partial class CAAAircraftGeneralDataScreen : ScreenControl
    {
        ...
        #region private CAAAircraftGeneralDataScreen()
        ///<summary>
        ///</summary>
        private CAAAircraftGeneralDataScreen()
        {
            InitializeComponent();
        }
        #endregion

        #region public CAAAircraftGeneralDataScreen(Aircraft currentAircraft)  : this()

        ///<summary>
        /// Создает страницу для отображения информации об одной директиве
        ///</summary>
        /// <param name="currentAircraft">Директива</param>
        public CAAAircraftGeneralDataScreen(Aircraft currentAircraft) : this()
        {
        ...
        void UpdateInformation()
        {
            ...
            if (_currentAircraft.ItemId > 0)
            {
                CurrentAircraft =
                    GlobalObjects.CaaEnvironment.NewLoader.GetObjectById<CAAAircraftDTO, Aircraft>(
                        _currentAircraft.ItemId, true);


                var equipment = GlobalObjects.CaaEnvironment.NewLoader.GetObjectList<CAAAircraftEquipmentDTO, AircraftEquipments>(new Filter("AircraftId", _currentAircraft.ItemId));
                CurrentAircraft.AircraftEquipments = new List<AircraftEquipments>(equipment);
