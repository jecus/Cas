...
namespace CAS.UI.UICAAControls.Event
{
    ...
    ///</summary>
    public partial class CAASmsEventTypeForm : MetroForm
    {
        private readonly Type _viewedType = typeof (CAAEventTypeRiskLevelChangeRecord);
        private readonly CAASmsEventType _smsEventType;
        ...
        private void AddPerformance()
        {
            ...
            {
                var newRecord =
                    new CAAEventTypeRiskLevelChangeRecord { ParentEventType = _smsEventType };
        ...
        public void FillControls()
        {
            ...
            if (_smsEventType != null)
            {
                ...
                foreach (var condition in _smsEventType.EventConditions)
                {
                    CAASmsConditionControl newControl = new CAASmsConditionControl(condition);
                ...
                foreach (var record in _smsEventType.ChangeRecords)
                {
        ...
        private bool ValidateData(out string message)
        {
            ...
            if (flowLayoutPanelConditions.Controls.OfType<CAASmsConditionControl>().Any(scc => !scc.CheckData()))
            {
                if (message != "") message += "\n ";
        ...
        private void Save()
        {
            try
            {
                GlobalObjects.NewKeeper.Save(_smsEventType);

                foreach (var eventCondition in _smsEventType.EventConditions)
                {
                    eventCondition.ParentId = _smsEventType.ItemId;
                    eventCondition.ParentType = _smsEventType.SmartCoreObjectType;
                    eventCondition.OperatorId = _smsEventType.OperatorId;
                    GlobalObjects.NewKeeper.Save(eventCondition);
                }

                foreach (var changeRecord in _smsEventType.ChangeRecords)
                {
                    changeRecord.ParentEventType = _smsEventType;
                    changeRecord.OperatorId = _smsEventType.OperatorId;
                    GlobalObjects.NewKeeper.Save(changeRecord);
                }
        ...
        private void LinkLabelAddNewConditionLinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            var performance = new CAASmsConditionControl(new CAAEventCondition {ParentId = _smsEventType.ItemId, OperatorId = _smsEventType.OperatorId});
        ...
        {
            CAASmsConditionControl control = (CAASmsConditionControl)sender;
