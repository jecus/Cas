	...
	public partial class PurchaseStatusListView : BaseGridViewControl<PurchaseRequestRecord>
	{
		...
		protected override List<CustomCell> GetListViewSubItems(PurchaseRequestRecord item)
		{
			...
			if (item.ParentInitialRecord != null)
				temp += $"| Name: {item.Product?.Name} | Destination: {destiantion} | Priority: {item.ParentInitialRecord?.Priority} | №:{((PurchaseOrder)item.ParentPackage).Number} | {SmartCore.Auxiliary.Convert.GetDateFormat(item.ParentPackage.OpeningDate)}";
			...
			return new List<CustomCell>
			{
				...
				CreateRow($"{SmartCore.Auxiliary.Convert.GetDateFormat(order.AdditionalInformation.ArrivalDate)} {order.AdditionalInformation.ArrivalTime:F1}", order),
				CreateRow($"{SmartCore.Auxiliary.Convert.GetDateFormat(order.AdditionalInformation.ReceiptDate)} {order.AdditionalInformation.ReceiptTime:F1}", order),
