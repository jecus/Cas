...
namespace CAS.UI.UIControls.PurchaseControls
{
    ...
    ///</summary>
    public partial class SelectAccessoryScheduleForm : MetroForm
    {
        #region Fields

        private readonly List<KeyValuePair<Component, NextPerformance>> _details = 
            new List<KeyValuePair<Component, NextPerformance>>();

        private readonly List<KeyValuePair<ComponentDirective, NextPerformance>> _detailDirectives = 
            new List<KeyValuePair<ComponentDirective, NextPerformance>>();
        ...
        ///</summary>
        public IEnumerable<ScheduleDirectiveProduct> Accessories
        {
            get
            {
                List<ScheduleDirectiveProduct> accessories = new List<ScheduleDirectiveProduct>();
                foreach (DataGridViewRow row in dataGridViewItems.Rows
                            .OfType<DataGridViewRow>()
                            .Where(r=>r.Tag is KeyValuePair<Component, NextPerformance> && r.ReadOnly == false))
                {
                    ...
                    KeyValuePair<Component, NextPerformance> d = (KeyValuePair<Component, NextPerformance>)row.Tag;
                    ...
                    {
                        accessories.Add(new ScheduleDirectiveProduct(d.Key, d.Value, d.Key.Product, 1));
                    }
                    if ((bool) kitCell.Value)
                    {
                        accessories.AddRange(d.Key.Kits.Select(k => new ScheduleDirectiveProduct(d.Key, d.Value, k.Product, k.Quantity)));
                    }
                }
                foreach (DataGridViewRow row in dataGridViewItems.Rows.OfType<DataGridViewRow>().Where(r => r.Tag is ComponentDirective))
                {
                    ...
                    KeyValuePair<ComponentDirective, NextPerformance> d = (KeyValuePair<ComponentDirective, NextPerformance>)row.Tag;

                    DataGridViewCheckBoxCell componentCell = (DataGridViewCheckBoxCell)row.Cells[1];
                    DataGridViewCheckBoxCell kitCell = (DataGridViewCheckBoxCell)row.Cells[2];

                    ComponentDirective dd = d.Key;
                    if ((bool)componentCell.Value &&
                        accessories.All(a => a.Product.ItemId != dd.ParentComponent.Product.ItemId))
                        accessories.Add(new ScheduleDirectiveProduct(dd.ParentComponent, d.Value, dd.ParentComponent.Product, 1));

                    if ((bool)kitCell.Value)
                        accessories.AddRange(dd.Kits.Select(k => new ScheduleDirectiveProduct(dd, d.Value, k.Product, k.Quantity)));
                }
                return accessories;
            }
        }
        ...
        ///<summary>
        ///</summary>
        private SelectAccessoryScheduleForm()
        {
            InitializeComponent();
        }
        ...
        ///<summary>
        ///</summary>
        public SelectAccessoryScheduleForm(IEnumerable<KeyValuePair<Component, NextPerformance>> details,
                                           IEnumerable<KeyValuePair<ComponentDirective, NextPerformance>> detailDirectives)
            : this()
        {
            if(_details == null)
                _details = new List<KeyValuePair<Component, NextPerformance>>();
            _details.AddRange(details);

            if (_detailDirectives == null)
                _detailDirectives = new List<KeyValuePair<ComponentDirective, NextPerformance>>();
        ...
        private void UpdateInformation()
        {
            ...
            foreach (KeyValuePair<Component, NextPerformance> detail in _details)
            {
                Component d = detail.Key;

                DataGridViewRow row = new DataGridViewRow {Tag = detail};
                DataGridViewCell discCell = new DataGridViewTextBoxCell {Value = d.ToString()};
                DataGridViewCell compntCell = new DataGridViewCheckBoxCell { Value = true };
                DataGridViewCell kitCell = new DataGridViewCheckBoxCell { Value = d.Kits.Count > 0 };
            ...
            foreach (KeyValuePair<ComponentDirective, NextPerformance> detailDirective in _detailDirectives)
            {
                ComponentDirective dd = detailDirective.Key;

                DataGridViewRow row = new DataGridViewRow { Tag = detailDirective };
                DataGridViewCell discCell = new DataGridViewTextBoxCell { Value = dd.ToString() };
                DataGridViewCell compntCell = new DataGridViewCheckBoxCell { Value = true };
                DataGridViewCell kitCell = new DataGridViewCheckBoxCell { Value = dd.Kits.Count > 0 };
        ...
        private bool ValidateData(out string message)
        {
                ...
                .Where(r => r.Tag is KeyValuePair<Component,NextPerformance> && r.ReadOnly == false))
            {
                ...
                KeyValuePair<Component, NextPerformance> detailPerformance = (KeyValuePair<Component, NextPerformance>)row.Tag;

                DataGridViewCheckBoxCell componentCell = (DataGridViewCheckBoxCell) row.Cells[1];
                DataGridViewCheckBoxCell kitCell = (DataGridViewCheckBoxCell) row.Cells[2];

                Component d = detailPerformance.Key;
            ...
            foreach (DataGridViewRow row in dataGridViewItems.Rows
                        .OfType<DataGridViewRow>()
                        .Where(r => r.Tag is KeyValuePair<ComponentDirective, NextPerformance>))
            {
                if (row.Cells.Count < 3
                    || !(row.Cells[1] is DataGridViewCheckBoxCell)
                    || !(row.Cells[2] is DataGridViewCheckBoxCell))
                ...
                KeyValuePair<ComponentDirective, NextPerformance> detailDirectivePerformance = 
                    (KeyValuePair<ComponentDirective, NextPerformance>)row.Tag;

                DataGridViewCheckBoxCell componentCell = (DataGridViewCheckBoxCell)row.Cells[1];
                DataGridViewCheckBoxCell kitCell = (DataGridViewCheckBoxCell)row.Cells[2];

                ComponentDirective dd = detailDirectivePerformance.Key;
        ...
        private void DataGridViewItemsCellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 1)
            {
                ...
                if(!(currentRow.Tag is KeyValuePair<ComponentDirective, NextPerformance>))
                    return;

                KeyValuePair<ComponentDirective, NextPerformance> detaildirectivePerformance = (KeyValuePair<ComponentDirective, NextPerformance>)currentRow.Tag;
                Component d = detaildirectivePerformance.Key.ParentComponent;
                DataGridViewRow row = dataGridViewItems.Rows
                    .OfType<DataGridViewRow>()
                    .FirstOrDefault(r => r.Tag is KeyValuePair<Component, NextPerformance> && 
                                        ((KeyValuePair<Component, NextPerformance>)r.Tag).Key.ItemId == d.ItemId);
                if (row != null)
                {
