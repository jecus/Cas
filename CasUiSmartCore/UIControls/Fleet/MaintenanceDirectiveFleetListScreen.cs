...
namespace CAS.UI.UIControls.Fleet
{
	...
	public partial class MaintenanceDirectiveFleetListScreen : ScreenControl
	{
		...
		public MaintenanceDirectiveFleetListScreen(Operator currentOperator, IEnumerable<ICommonFilter> initialFilters = null)
			: this()
		{
		...
		private void HeaderControlButtonPrintDisplayerRequested(object sender, ReferenceEventArgs e)
		{
			...
			if (sender == itemPrintReportAMP)
			{
				...
				GlobalObjects.AuditRepository.WriteReportAsync(GlobalObjects.CasEnvironment.IdentityUser, "MaintenanceDirectiveFleetListScreen (AMP STATUS)");
			}
			else if (sender == itemPrintReportMP)
			{
				...
				GlobalObjects.AuditRepository.WriteReportAsync(GlobalObjects.CasEnvironment.IdentityUser, "MaintenanceDirectiveFleetListScreen (MP)");
			}
			else if (sender == itemPrintReportMPLimit)
			{
				...
				GlobalObjects.AuditRepository.WriteReportAsync(GlobalObjects.CasEnvironment.IdentityUser, "MaintenanceDirectiveFleetListScreen (MP Limit)");
			}
			else if (sender == itemPrintWorkscope)
			{
				...
				GlobalObjects.AuditRepository.WriteReportAsync(GlobalObjects.CasEnvironment.IdentityUser, "MaintenanceDirectiveFleetListScreen (WORK SCOPE)");
			}
			else if (sender == itemPrintMPSYS)
			{
				...
				GlobalObjects.AuditRepository.WriteReportAsync(GlobalObjects.CasEnvironment.IdentityUser, "MaintenanceDirectiveFleetListScreen (MP SYS)");
			}
			else if (sender == itemPrintMPSTR)
			{
				...
				e.RequestedEntity = new ReportScreen(_amReportBuilder);
				GlobalObjects.AuditRepository.WriteReportAsync(GlobalObjects.CasEnvironment.IdentityUser, "MaintenanceDirectiveFleetListScreen (MP STR)");
			}
			else if (sender == itemPrintMPZonal)
			{
				...
				e.RequestedEntity = new ReportScreen(_amReportBuilder);
				GlobalObjects.AuditRepository.WriteReportAsync(GlobalObjects.CasEnvironment.IdentityUser, "MaintenanceDirectiveFleetListScreen (MP Zonal)");
			}
			else if (sender == itemPrintMPAWL)
			{
				...
				e.RequestedEntity = new ReportScreen(_amReportBuilder);
				GlobalObjects.AuditRepository.WriteReportAsync(GlobalObjects.CasEnvironment.IdentityUser, "MaintenanceDirectiveFleetListScreen (MP AWL + CMR + FIS)");
			}
			else if (sender == itemPrintMPLine)
			{
				...
				e.RequestedEntity = new ReportScreen(_amReportBuilder);
				GlobalObjects.AuditRepository.WriteReportAsync(GlobalObjects.CasEnvironment.IdentityUser, "MaintenanceDirectiveFleetListScreen (MP Operator)");
			}
			else if (sender == itemPrintMPSSIP)
			{
				...
				GlobalObjects.AuditRepository.WriteReportAsync(GlobalObjects.CasEnvironment.IdentityUser, "MaintenanceDirectiveFleetListScreen (MP SSIP)");
			}
			else if (sender == itemPrintLitAvia)
			{
				...
				e.RequestedEntity = new ReportScreen(_maintenanceDirectiveReportBuilderLitAVia);
				GlobalObjects.AuditRepository.WriteReportAsync(GlobalObjects.CasEnvironment.IdentityUser, "MaintenanceDirectiveFleetListScreen (SS LA)");
			}
			else
			{
				e.Cancel = true;
		...
		private void CopyToClipboard()
		{
			...
			try
			{
				DataFormats.Format format = DataFormats.GetFormat(typeof(MaintenanceDirective[]).FullName);

				if (_directivesViewer.SelectedItems == null || _directivesViewer.SelectedItems.Count == 0)
					return;

				List<MaintenanceDirective> pds = new List<MaintenanceDirective>();
				foreach (MaintenanceDirective maintenanceDirective in _directivesViewer.SelectedItems)
				{
					pds.Add(maintenanceDirective.GetCopyUnsaved());
				}
		...
		private void GetFromClipboard()
		{
			try
			{
				string format = typeof(MaintenanceDirective[]).FullName;

				if (string.IsNullOrEmpty(format))
					return;
				if (!Clipboard.ContainsData(format))
					return;
				var pds = (MaintenanceDirective[])Clipboard.GetData(format);
				if (pds == null)
					return;

				var objectsToPaste = new List<MaintenanceDirective>();
				var bd = CurrentParent is BaseComponent ? (BaseComponent)CurrentParent : GlobalObjects.ComponentCore.GetBaseComponentById(((Aircraft)CurrentParent).AircraftFrameId);
				foreach (MaintenanceDirective md in pds)
				{
					md.ParentBaseComponent = bd;

					GlobalObjects.PerformanceCalculator.GetNextPerformance(md);
					_initialDirectiveArray.Add(md);
					_resultDirectiveArray.Add(md);

					md.TaskNumberCheck += " Copy";
					objectsToPaste.Add(md);
				}
